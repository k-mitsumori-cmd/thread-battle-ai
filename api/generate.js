...
【ウィットに富んだ論破のコツ】
- 予想外の角度からのツッコミ（例：「それってつまり、お前は空気読めないってこと？」）
- ユーモアのある比喩を使う（例：「お前の論理、迷路みたいにぐるぐる回ってるなｗ」）
- 逆説的な指摘（例：「お前が正しいって言ってる時点で、お前が間違ってる」）
- 具体的な例え話（例：「それって、魚が水の存在を否定してるようなもんだろ」）
- 論理の飛躍を面白おかしく指摘（例：「A→B→Cって言ってるけど、お前の頭の中ではA→Zに飛んでるだろｗ」）
- 自虐的なツッコミも混ぜる（例：「まぁ、お前も大概だけどなｗ」）
- 単なる否定ではなく、機知に富んだ反論を心がける`
                },
                {
                    role: 'user',
                    content: prompt
...
以下のテクニックを使って、ウィットに富んだ論破を心がけてください：
- 予想外の角度からのツッコミ（例：「それってつまり、お前は空気読めないってこと？」）
- ユーモアのある比喩（例：「お前の論理、迷路みたいにぐるぐる回ってるなｗ」）
- 逆説的な指摘（例：「お前が正しいって言ってる時点で、お前が間違ってる」）
- 具体的な例え話（例：「それって、魚が水の存在を否定してるようなもんだろ」）
- 論理の飛躍を面白おかしく指摘（例：「A→B→Cって言ってるけど、お前の頭の中ではA→Zに飛んでるだろｗ」）`;
...
よりウィットに富んだ、機知に満ちた反論を心がけてください。予想外の角度からのツッコミや、ユーモアのある比喩を使うと良いでしょう。`;
